...
srv_sd  <- sqrt(log(1+(srv_se/srv_est)^2))
## Convert to TMB inputs
data <- list(yrs=styr:endyr,
             yrs_srv=yrs_srv,
...
## Compare to RE model in ADMB (copied manually from output
## file). Strangely when I rerun it now I get a different result
## and it doesn't match TMB either. But I didn't change the RE model??
...
round((admb$biomsd-tmb$logbiomass)/admb$biomsd,6)
round((admb$biomsd.sd-tmb$logbiomass.sd)/admb$biomsd.sd,6)
