 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  logSdLam.allocate(2,"logSdLam");
  mu.allocate(1,"mu");
  biomsd.allocate(styr,endyr,"biomsd");
  u.allocate(styr,endyr,2,"u");
...
void model_parameters::userfunction(void)
{
  ...
  for(int i=styr+1; i<=endyr; ++i)
  {
    ...
    jnll += logSdLam;
    jnll += 0.5*(square(sig*(u(i-1) - u(i))/logSdLam));
    // cout<<jnll<<endl;
  }
  for(int i=1; i<=nobs; ++i)
  {
    ...
    // Normal version
    jnll+=0.5*(yconst(i) + square( (mu + sig*u(yrs_srv(i))) - srv_est(i))/yvar(i));
  }
  if (sd_phase()) biomsd = mu  + sig*u;
  /*
